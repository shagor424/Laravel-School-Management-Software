...
class StudentTutionFeeController extends Controller
{
    public function view(){
    	...
    	return view('backend.student.student_tutionfee.view-tutionifee',$data);

    }

    public function getstutionfee(Request $request){
    	...
    	$html['thsource'] .= '<th>Tution Fee</th>';
    	$html['thsource'] .= '<th>Discount</th>';
    	$html['thsource'] .= '<th>Fee (This Student)</th>';
    	$html['thsource'] .= '<th>Action</th>';

    	foreach ($allstudent as $key => $v) {
    	$regifee = FeeAmount::where('fee_catagory_id','3')->where('class_id',$v->class_id)->first();
    		$color = 'success';
    		...
    		$html[$key]['tdsource'] .=' <a class="btn btn-sm btn-'.$color.'" title="tution payslip" target="_blank" href="'.route("students.tutionfee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'">Tution Fee Slip</a>';
    		$html[$key]['tdsource'] .='</td>';
    	}
    ...
    public function tutionpayslip(Request $request){

    	$student_id = $request->student_id;
    	$class_id = $request->class_id;
    	$allstudent['details'] = AssignStudent::with(['disscount','student'])->where('student_id',$student_id)->where('class_id',$class_id)->first(); 
    	
    	$pdf = PDF::loadView('backend.student.student_tutionfee.tutionfee-pdf',$allstudent);
    	$pdf->setProtection(['copy','print'], '','pass');
    	return $pdf->stream('student-tution-fee.pdf');
